...
1 --> 1st reached target, 2nd reached target
2 --> 1st reached target, 2nd did not reach target
3 --> 1st did not reach target, 2nd reached target
4 --> 1st did not reach target, 2nd did not reach target
'''
...
'''
Choose two initial points for the pixel to travel 
'''
...
'''
1st pair
Generates two curves ---> First is for the target and the second is for the distractor where its end point is used as the red box for negative samples
...
def get_full_length_coordinates(nPoints, nTimes, normalized, path_length, skip_param=skip_param):
    ...
    But why 1/3 and not all simply?
    '''
...
##    del coordinates_3[-2]
    ...
    '''
    cd_4=GenCoord()
    coordinates_4=cd_4.get_coordinates(length_curve=nTimes, angle_range=[90,100], distance_points=.002, delta_angle_max=90,wiggle_room=.95,rigidity=.95, x_min=0,x_max=win_x,y_min=0,y_max=win_y)
    ...
    But why 1/3 and not all simply?
    '''
    rn_l=np.random.randint(len(coordinates_4)-(path_length*skip_param)+1)
    coordinates_4=coordinates_4[rn_l:rn_l+(path_length*skip_param):skip_param]
##    del coordinates_2[1]
##    del coordinates_2[1]
##    del coordinates_2[-2]
##    del coordinates_2[-2]
...
##    del coordinates_2[-2]
    ...
    if normal[0]:
        #coordinates_2=[((coordinates_2[i][0]-(win_x/float(normal[0]))),(coordinates_2[i][1]-(win_y/float(normal[0])))) for i in range(0,len(coordinates_2))]
        coordinates_4=[((coordinates_4[i][0]-(float(normal[0]))),(coordinates_4[i][1]-(float(normal[0])))) for i in range(0,len(coordinates_4))]
    ...
    '''
    cd_5 = GenCoord()
    coordinates_5 = cd_5.get_coordinates(length_curve=nTimes, angle_range=[90,100], distance_points=.002, delta_angle_max=90,wiggle_room=.95,rigidity=.95, x_min=0,x_max=win_x,y_min=0,y_max=win_y)
    ...
    But why 1/3 and not all simply?
    '''
    rn_l=np.random.randint(len(coordinates_5)-(path_length*skip_param)+1)
    coordinates_5=coordinates_5[rn_l:rn_l+(path_length*skip_param):skip_param]
    ##    del coordinates_3[1]
    ##    del coordinates_3[1]
    ##    del coordinates_3[-2]
    ##    del coordinates_3[-2]
    ...
    if normal[1]:
        #coordinates_3=[((coordinates_3[i][0]-(win_x/float(normal[1]))),(coordinates_3[i][1]-(win_y/float(normal[1])))) for i in range(0,len(coordinates_3))]
        coordinates_5 = [((coordinates_5[i][0]-(float(normal[1]))),(coordinates_5[i][1]-(float(normal[1])))) for i in range(0,len(coordinates_5))]
    ...
    return np.floor(coordinates_2), np.floor(coordinates_3), np.floor(coordinates_4), np.floor(coordinates_5), normalized
...
path=args.path+"/"+str(int(args.LABEL_MODE))
...
for sample in range(start_sample,num_samples):
    ...
    if args.save_image or args.gif:
        path_to_save=path+"/"+str(int(args.LABEL_MODE))+"_sample_"+str(sample)+"/"
    ...
    # White mark and yellow mark the path of the targets
    if args.HUMAN_MODE:
        images_human=np.zeros((path_length,32,32,channels), dtype=np.uint8)
        images_gif_human = []

        for humane_frame in range(0,path_length):
            # 2
            rr, cc = rectangle(coordinates_2[humane_frame][0]+1,coordinates_2[humane_frame][1]+1,square_height,square_width)
            images_human[humane_frame, rr, cc] = (255,255,255)
            # 4
            rr, cc = rectangle(coordinates_4[humane_frame][0]+1,coordinates_4[humane_frame][1]+1,square_height,square_width)
            images_human[humane_frame, rr, cc] = (255,255,0)

            if humane_frame != 0:
                images_human[humane_frame] = images_human[humane_frame] + images_human[humane_frame-1]
                images_human = np.clip(images_human, a_min = 0, a_max = 255)

        for humane_frame in range(0,path_length-1):
            images_human[humane_frame] = images_human[path_length-1]
    ...
    for frame in range(0,path_length):
        ...
        images[frame,rr, cc] = (255,0,0) #128 #red starting open square
        # images[frame,rr, cc] = (255,182,193) #128 #red starting open square
        if args.HUMAN_MODE:
            images_human[frame,rr, cc] = (255,0,0)
            # images_human[frame,rr, cc] = (255,182,193)

        # draw the end of path
        # 1 --> 1st reached target #2 , 2nd reached target #4
        if args.LABEL_MODE == 1:
            # 2
            rr, cc = draw_blobs(coordinates_2[path_length-1],blob_height,blob_width)
            images[frame,rr, cc] = (0, 0, 255)
            if args.HUMAN_MODE:
                images_human[frame,rr, cc] = (0,0,255)
            # 4
            rr, cc = draw_blobs(coordinates_4[path_length-1],blob_height,blob_width)
            images[frame,rr, cc] = (0, 0, 255)
            # images[frame,rr, cc] = (112, 128, 144)
            if args.HUMAN_MODE:
                images_human[frame,rr, cc] = (0,0,255)
                # images_human[frame,rr, cc] = (112, 128, 144)

        # 1 --> 1st reached target #2 , 2nd did not reach target #5
        ...
        # 1 --> 1st did not reach target #3 , 2nd reached target #4
        elif args.LABEL_MODE == 3:
            # 3
        ...
        # 1 --> 1st did not reach target #3 , 2nd did not reach target #5
        elif args.LABEL_MODE == 4:
            # 3
            rr, cc = draw_blobs(coordinates_3[path_length-1],blob_height,blob_width)
            images[frame,rr, cc] = (0, 0, 255)
            if args.HUMAN_MODE:
        ...
        images[frame, rr, cc] = (0,255,0)
        if args.HUMAN_MODE:
            images_human[frame,rr, cc] = (0,255,0)

        rr, cc = rectangle(coordinates_3[frame][0]+1,coordinates_3[frame][1]+1,square_height,square_width)
        images[frame, rr, cc] = (0,255,0)

        rr, cc = rectangle(coordinates_4[frame][0]+1,coordinates_4[frame][1]+1,square_height,square_width)
        images[frame, rr, cc] = (0,255,0)
        if args.HUMAN_MODE:
            images_human[frame,rr, cc] = (0,255,0)

        rr, cc = rectangle(coordinates_5[frame][0]+1,coordinates_5[frame][1]+1,square_height,square_width)
        images[frame, rr, cc] = (0,255,0)
        ...
        if args.HUMAN_MODE:
            # Extrapolating 8x for GIFs
            repeat_factor=8
            images_human_frame = images_human[frame].repeat(repeat_factor, axis = 0).repeat(repeat_factor, axis = 1)
            images_gif_human.append(images_human_frame)
    ...
    np.save(path+"/"+str(int(args.LABEL_MODE))+"_sample_"+str(sample), images, allow_pickle=False)
    ...
    # SAVING GIFs
    if args.gif:
        imageio.mimsave(path_to_save+"/GIF.gif", images_gif, fps = 20)

        if args.HUMAN_MODE:
            imageio.mimsave(path_to_save+"/GIF_human.gif", images_gif_human, fps = 20)
