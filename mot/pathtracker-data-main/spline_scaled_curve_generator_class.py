...
class GenCoord():
    ...
    def get_coordinates(self,nPoints=1,length_curve=150, distance_points=0.001,angle_range=[10,30],delta_angle_max=5,wiggle_room=0.95,rigidity=0.975,x_min=-115.0,x_max=115.0,y_min=-115.0,y_max=115.0,draw_plot=False):
        ...
        wiggle_room=0.95 #should be non-zero and less than delta_angle_max. Controls smoothness (by variation in choice of angle), \
                         larger the number, smoother the curve ---> how smoother?
        ...
        self.angle_range = angle_range #range of angles at which the curve can bend at any point while sampling ---> Not being used but
        ...
        x.append(a[0])
        y.append(a[1])
        # a[1] --> Y Coordinates and a[0] --> X Coordinates
        ...
        for i in range(self.length_curve):
            ...
            # np.arange --> Return evenly spaced values within a given interval. Values are generated within the half-open interval [start, stop)
