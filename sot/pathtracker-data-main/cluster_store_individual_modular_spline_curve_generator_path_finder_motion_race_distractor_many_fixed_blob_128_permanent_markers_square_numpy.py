...
parser.add_argument('-n', '--num_samples', type=int, default=3, help="int: number of samples to be generated [default 10]")
parser.add_argument('-ss', '--start_sample', type=int, default=0, help="int: number at which to start the sample numbering [default 0]")
parser.add_argument('-nd', '--num_distractors', type=int, default=14, help="int: number of distractor paths [default 10]")
...
parser.add_argument('-p', '--path', type=str, default="/users/aarjun1/data/aarjun1/pathtracker/sot/pathtracker-data-main/outer_path/" \
...
'''
Choose two initial points for the pixel to travel --> THis is not used though maybe?
'''
...
for sample in range(start_sample,num_samples):
    ...
    for frame in range(0,path_length):
        ...
        images_gif.append(images[frame])
    ...
    if args.gif:
        imageio.mimsave(path_to_save+"/GIF.gif", images_gif)
